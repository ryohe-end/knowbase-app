...
const ymdToMs = (s: string) => (/^\d{8}$/.test(s) ? Date.UTC(+s.slice(0, 4), +s.slice(4, 6) - 1, +s.slice(6, 8)) : NaN);
...
function Inner() {
  ...
  return (
    <div className="jot-root">
      ...
      <header className="jot-header">
        <div className="jot-header-inner">
          <Link href="/store-settings" className="jot-back">← メニューへ戻る</Link>
          <h1>JOYFIT OneTimePass金額設定<span className="jot-badge">{isAdmin ? "全店" : "自店舗"}</span></h1>
          <span className="jot-src">EnjoyTimePass (t1pass)</span>
        </div>
      </header>

      <main className="jot-main">
        ...
        <section className="jot-edit">
          {!selected ? <div className="jot-edit-empty">左の一覧から店舗を選んでください。</div>
            ...
            : (
              <>
                <div className="jot-edit-head"><div><code className="jot-club-id">{selected}</code><h2>{clubName}</h2></div></div>
      ...
      <style jsx global>{`
        .jot-root { background: #f8fafc; min-height: 100vh; font-family: sans-serif; color: #0f172a; }
        .jot-header { background: #fff; border-bottom: 1px solid #e2e8f0; position: sticky; top: 0; z-index: 50; }
        .jot-header-inner { max-width: 1200px; margin: 0 auto; padding: 14px 24px; display: flex; align-items: center; gap: 16px; }
        .jot-back { text-decoration: none; font-size: 13px; font-weight: 600; color: #64748b; }
        .jot-header h1 { font-size: 18px; font-weight: 800; margin: 0; display: flex; align-items: center; gap: 8px; }
        .jot-badge { font-size: 10px; font-weight: 800; color: #6d28d9; background: #ede9fe; border: 1px solid #ddd6fe; padding: 2px 8px; border-radius: 99px; }
        .jot-src { margin-left: auto; font-size: 11px; color: #94a3b8; font-family: monospace; }
        .jot-main { max-width: 1200px; margin: 0 auto; padding: 20px 24px; display: grid; grid-template-columns: 340px 1fr; gap: 18px; }
        ...
        .jot-edit-head { margin-bottom: 16px; }
