...
const ymdToNum = (s: string) => Number(s);
// YYYYMMDD -> ミリ秒(比較/描画用)
...
function Inner() {
  ...
  return (
    <div className="f1d-root">
      ...
      <header className="f1d-header">
        <div className="f1d-header-inner">
          <Link href="/store-settings" className="f1d-back">← メニューへ戻る</Link>
          <h1>FIT365 1dayパス金額設定<span className="f1d-badge">{isAdmin ? "全店" : "自店舗"}</span></h1>
          <span className="f1d-src">fit365sf (entry)</span>
        </div>
      </header>

      <main className="f1d-main">
      ...
      <style jsx global>{`
        .f1d-root { background: #f8fafc; min-height: 100vh; font-family: sans-serif; color: #0f172a; }
        .f1d-header { background: #fff; border-bottom: 1px solid #e2e8f0; position: sticky; top: 0; z-index: 50; }
        .f1d-header-inner { max-width: 1200px; margin: 0 auto; padding: 14px 24px; display: flex; align-items: center; gap: 16px; }
        .f1d-back { text-decoration: none; font-size: 13px; font-weight: 600; color: #64748b; }
        .f1d-header h1 { font-size: 18px; font-weight: 800; margin: 0; display: flex; align-items: center; gap: 8px; }
        .f1d-badge { font-size: 10px; font-weight: 800; color: #b45309; background: #fef3c7; border: 1px solid #fde68a; padding: 2px 8px; border-radius: 99px; }
        .f1d-src { margin-left: auto; font-size: 11px; color: #94a3b8; font-family: monospace; }
        .f1d-main { max-width: 1200px; margin: 0 auto; padding: 20px 24px; display: grid; grid-template-columns: 360px 1fr; gap: 18px; }
