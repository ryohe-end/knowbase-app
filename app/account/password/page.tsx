...
function PasswordPageContent() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      setMsg(
        "保存はできましたが、強制フラグの解除に失敗しました。管理者に連絡してください。"
      );
    } catch (err) {
      console.error(err);
      setMsg("通信エラーが発生しました");
  ...
  return (
    <div className="kb-root">
      <div className="kb-topbar">
        ...
        <div style={{ fontSize: 18, fontWeight: 800 }}>パスワードの変更</div>

        {/* ✅ 戻るボタンは「トップ右に1つ」だけ。mustChange中も confirm 付きで戻れる */}
        <div className="kb-topbar-right">
          <button
            ...
            onClick={() => {
              if (mustChange) {
                const ok = confirm(
                  "初回ログインのためパスワード変更が必要です。\n戻るとログアウトしてログイン画面に戻ります。\nよろしいですか？"
                );
                if (!ok) return;
              ...
              <>
                <div
                  style={{
                    ...
                    background:
                      "linear-gradient(180deg, rgba(2,132,199,0.06), rgba(255,255,255,0.90))",
                    marginBottom: 14,
                  }}
                >
                  ...
                  {!!msg && (
                    <div
                      style={{
                        ...
                        border: "1px solid rgba(15,23,42,0.10)",
                        background:
                          tone === "ok" ? "rgba(14,165,233,0.08)" : "rgba(239,68,68,0.08)",
