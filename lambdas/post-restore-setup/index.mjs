...
async function setup(adminCfg, roCfg) {
  ...
  try {
    ...
    const roTables = [
      ...
      "会員契約明細", "契約形態",
      // 公開API(club-fees): クラブ別の契約会費金額(適用年月付き)
      "契約会費金額",
      // 返金・入金(返金画面): 口座情報
      "会員契約者口座",
      // ビーコン日次同期用 (ゲート/ビーコン系)
      ...
      // 貸倒対象照合(経理連携CSV: writeoff_reconcile)。会員別の未納/売掛/売上/入金/入金方式別/最終残高。
      // 振替契約者別(契約者×振替年月の一意化), 個人(既出), 売上入金/入金方式明細/精算(入金方式別内訳)。
      "振替契約者別", "売上入金", "入金方式明細", "精算",
