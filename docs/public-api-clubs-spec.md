...
外部システム向け公開API。KnowBase に登録されたクラブ（店舗）の一覧を返す。**閉店店舗も含めて**返し、各店に閉店フラグが付く。各店に**経営企業コード**と**企業名**が付く。
...
```
GET /api/public/clubs[?clubCode=][&brand=][&companyCode=][&includeClosed=0][&formCodes=][&formMatch=]
...
| `companyCode` | – | number | 経営企業コードで絞り込み（例: `1`=オカモト、`1002`=ヤマウチ 等） |
...
> 法人の特定主契約を扱い、かつ特定の経営企業に属する店舗だけを出したい場合は `formCodes` と `companyCode` を併用できる。
...
> `managementCompanyCode` / `companyName` は常に返す（未設定店は `null`）。

### 200 OK（`clubCode` で1店指定時）

```json
{ "ok": true, "club": { "clubCode": "375", "clubName": "…", "brand": "JOYFIT", "businessType": "赤", "closed": false, "managementCompanyCode": "1", "companyName": "オカモト" } }
```
...
# 経営企業コードで絞り込み
curl -H "x-api-key: $KEY" "https://<host>/api/public/clubs?companyCode=1"

# 特定の契約形態（主契約）を扱う店舗だけ（法人入会用）: いずれか
curl -H "x-api-key: $KEY" "https://<host>/api/public/clubs?formCodes=2052,711"

# 指定コードを全て扱う店舗のみ + 特定の経営企業
curl -H "x-api-key: $KEY" "https://<host>/api/public/clubs?formCodes=2052,711&formMatch=all&companyCode=1"
```
...
- 契約形態フィルタの判定源 `FIT_ADMIN.契約会費金額`、経営企業コード源 `FIT_ADMIN.クラブ` はいずれも adb01 が毎晩リストアされるため、`knowbie_ro` への
  `GRANT SELECT` を `lambdas/post-restore-setup/index.mjs` の `roTables` に登録済み（`契約会費金額`・`クラブ`ともに登録済／リストア後も維持）。
- member-search Lambda（`knowbie-member-search`）に `type:"clubs-list"`（`formCodes`/`formMatch` 対応・経営企業コード JOIN）ハンドラを実装。
  **ハンドラ更新時は Lambda の再デプロイが必要**（`cd lambdas/member-search && zip function.zip index.mjs && aws lambda update-function-code --region ap-northeast-1 --function-name knowbie-member-search --zip-file fileb://function.zip`）。
